...
const Profile = () => {
  ...
  return (
    <div style={containerStyle}>
        ...
        <h3 style={{color:'#f97316', marginBottom: '2opx', fontSize: '1.5rem'}}>Order History</h3>
       {loading ? (
          ...
          {orders.map(order => (
            <div key={order._id} style={{ background: '#09090b', padding: '20px', borderRadius: '12px', border: '1px solid #27272a', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '20px' }}>
              <div>
                ...
                <p style={{ color: '#a1a1aa', fontSize: '0.9rem' }}>Total: <strong style={{ color: '#10b981' }}>₹{order.totalAmout.toFixed(2)}</strong></p>
              </div>
              <div>
                <span style={{ 
