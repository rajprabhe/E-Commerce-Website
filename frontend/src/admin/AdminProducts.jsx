        ...
        <table style={tableStyle}>
          ...
          <tbody>
            {products.map(products => (
              <tr key={products._id} style={rowStyle}>
                <td style={tdStyle}>{products._id.substring(0,8)}...</td>
                <td style={tdStyle}>{products.name}</td>
                <td style={tdStyle}>{products.price.toFixed(2)}</td>
                <td style={tdStyle}>{products.category}</td>
                <td style={tdStyle}>{products.stock}</td>
                <td style={tdStyle}>
                  <Link to={`/admin/edit-product/${products._id}`} style={editBtn}>Edit</Link>
                  <button onClick={() => handleDelete(products._id)} style={deleteBtn}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
